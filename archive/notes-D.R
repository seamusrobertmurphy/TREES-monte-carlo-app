...
# ENHANCED: Hyperparameter optimization for uncertainty reduction
# ------------------------------------------------------------------------ #
...
# ENHANCED: Monte Carlo simulation with hyperparameter optimization
run_enhanced_monte_carlo <- function(data, hfld_crediting_level, n_iterations = 10000, 
  ...
  if(requireNamespace("caret", quietly = TRUE)) {
    tryCatch({
      ...
      # Prepare comprehensive dataset for hyperparameter optimization
      tuning_data <- data.frame(
        emissions = data$AD_Mean * data$EF_Mean,
        ad_mean = data$AD_Mean,
        ef_mean = data$EF_Mean,
        ad_ci = data$AD_CI_90_percent,
        ef_ci = data$EF_CI_90_percent,
        ad_se = (data$AD_Mean * data$AD_CI_90_percent / 100) / 1.645006,
        ef_se = (data$EF_Mean * data$EF_CI_90_percent / 100) / 1.645006,
        combined_uncertainty = sqrt((data$AD_CI_90_percent)^2 + (data$EF_CI_90_percent)^2),
        uncertainty_ratio = data$AD_CI_90_percent / (data$EF_CI_90_percent + 0.01),
        emission_intensity = (data$AD_Mean * data$EF_Mean) / sum(data$AD_Mean * data$EF_Mean),
        log_ad_mean = log(data$AD_Mean + 1),
        log_ef_mean = log(data$EF_Mean + 1)
      )
      
      # Monte Carlo cross-validation setup (ART-TREES compliant)
      mc_control <- trainControl(
        method = "LGOCV",         # Leave Group Out CV for Monte Carlo
        number = cv_folds,
        p = 0.8,
      ...
      if(enable_preprocessing) {
        preprocess_options <- c("center", "scale", "YeoJohnson")
      }
      
      # Feature selection
      formula_features <- if(enable_feature_selection) {
        emissions ~ ad_mean + ef_mean + ad_ci + ef_ci + combined_uncertainty + uncertainty_ratio + emission_intensity
      } else {
        emissions ~ ad_mean + ef_mean + ad_ci + ef_ci
      }
      
      # Model-specific hyperparameter grids
      tune_grid <- switch(model_method,
        "lm" = NULL,
        "rf" = expand.grid(mtry = seq(2, min(ncol(tuning_data)-1, 8), length.out = tune_length)),
        "gbm" = expand.grid(
          n.trees = c(50, 100, 150)[1:min(3, tune_length)],
          interaction.depth = c(1, 3, 5)[1:min(3, tune_length)],
          shrinkage = 0.1,
          n.minobsinnode = 10
        ),
        "svmRadial" = expand.grid(
          sigma = c(0.01, 0.1, 1)[1:min(3, tune_length)],
          C = c(0.5, 1, 2)[1:min(3, tune_length)]
        ),
        "glmnet" = expand.grid(
          alpha = seq(0, 1, length.out = tune_length),
          lambda = 10^seq(-3, 0, length.out = tune_length)
        )
      )
      ...
      mean_emissions <- mean(tuning_data$emissions, na.rm = TRUE)
      
      # Calculate bias correction factor
      if(mean_emissions > 0) {
        bias_correction_factor <- 1 - (best_performance / mean_emissions)
        bias_correction_factor <- max(0.90, min(1.10, bias_correction_factor))
      }
      ...
      tuning_results <- list(
        ...
        variable_importance = if(model_method %in% c("rf", "gbm")) varImp(tuning_model) else NULL
      )
      
      tuning_applied <- TRUE
      
      cat("Hyperparameter Optimization Results:\n")
      ...
      cat("Hyperparameter optimization failed:", e$message, "\n")
      cat("Using standard Monte Carlo simulation\n")
    })
  }
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    # CV Parameters
    numericInput("cv_folds", "CV Folds:", value = 10, min = 3, max = 20, step = 1, width = "90%"),
    
    numericInput("tune_length", "Hyperparameter Grid Size:", value = 3, min = 1, max = 10, step = 1, width = "90%"),
    
    checkboxInput("enable_preprocessing", "Enable Preprocessing", value = TRUE, width = "90%"),
    
    checkboxInput("enable_feature_selection", "Enhanced Feature Selection", value = TRUE, width = "90%"),
        ...
        fluidRow(
          box(
            ...
            tags$ul(
              ...
              tags$li("Cross-Validation: LGOCV with configurable folds"),
              tags$li("Preprocessing: Centering, scaling, Yeo-Johnson transformation"),
              tags$li("Feature Engineering: Enhanced uncertainty metrics"),
...
# Enhanced Server Logic with Hyperparameter Optimization
server <- function(input, output, session) {
  ...
  # Reset hyperparameters to optimized defaults
  observeEvent(input$reset_hyperparameters, {
    updateSelectInput(session, "model_method", selected = "rf")
    updateSelectInput(session, "performance_metric", selected = "RMSE")
    updateNumericInput(session, "cv_folds", value = 10)
    updateNumericInput(session, "tune_length", value = 3)
    updateCheckboxInput(session, "enable_preprocessing", value = TRUE)
    updateCheckboxInput(session, "enable_feature_selection", value = TRUE)
    
    showNotification("Hyperparameters reset to optimized defaults", type = "message")
  })
  ...
  output$cv_summary <- renderText({
    ...
    if(results$tuning_applied) {
      paste0(
        ...
        "This optimization reduces uncertainty and improves\nthe reliability of ART TREES calculations."
      )
    } else {
      "No cross-validation optimization applied.\nStandard Monte Carlo simulation used."
    }
  })
  
  # All other existing output functions remain the same...
  # (monte_carlo_input_table, carbon_stocks_table, activity_data_table, 
  #  uncertainty_summary, stratum_results, calculation_details, enhanced_sensitivity_analysis)
  ...
  output$calculation_details <- renderText({
    ...
    paste0(
      ...
      "✓ SUCCESS: Net ERRs optimized through advanced modeling techniques!"
    )
  })
